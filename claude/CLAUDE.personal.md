...
- **Agent `name` = delivery-mode switch (replay-proven 2026-07-01):** naming an `Agent`/`Task` call flips it from fire-and-forget (final report auto-returned as the tool result) to teammate/mailbox mode (report NOT auto-returned; idle agents need SendMessage nudges). Dispatch independent parallel subagents UNNAMED; pass `name` only with a concrete reason to message that agent mid-run (then consult `agent-teams`). This bullet is the decision-time layer: `bin/replay-eval` showed Fable repeats this miss from real context without it, and the PreToolUse advisory hook alone fires too late to stop a same-turn parallel dispatch.
- **When to use /deep-research vs a single research agent** (validated 2026-07-01: the workflow extracted a decisive government stat a solo opus agent had only flagged as "exists, unextracted"): use a single agent for scoping passes ("does data exist? what shape?"); use the /deep-research workflow (or equivalent multi-agent verified harness, models set explicitly) when the answer is evidence-critical, will be cited/defended, or the first pass says "the data exists but I didn't get it." The verify stage (adversarial refute-votes) is what makes results citable.
