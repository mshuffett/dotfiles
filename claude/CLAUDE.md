...
When creating or modifying skills, always invoke the `skill-creator` skill first (the official Anthropic plugin version with eval/improve/benchmark modes). Use it for drafting, testing with evals, and iterative improvement. Canonical entrypoint skills live in `~/.dotfiles/agents/skills/` and must be committed (with `~/.dotfiles/claude/skills/` as a compatibility symlink).
