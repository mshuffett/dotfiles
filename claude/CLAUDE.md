...
When creating or modifying skills, always invoke `skill-creator:skill-creator` first (the official Anthropic plugin — not the Codex `.system` one). Use it for drafting, testing with evals, and iterative improvement. Canonical entrypoint skills live in `~/.dotfiles/agents/skills/` and must be committed (with `~/.dotfiles/claude/skills/` as a compatibility symlink).
