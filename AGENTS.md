...
- **`Codex/`** - Codex configuration including commands and hooks
...
- AI assistants: codecompanion.nvim and avante.nvim using Codex subscription
...
Codex, c      # Codex with --dangerously-skip-permissions
p              # pnpm
```
...
idea <text>                         # Quick idea capture using Codex /idea command
```
...
Codex configuration is in `Codex/`:
- `AGENTS.md` - Project instructions (this file)
- `settings.json` - Codex settings
- `commands/` - Custom slash commands
- `scripts/` - Hook scripts

Session history stored at: `~/.Codex/projects/[project-path]/[session-uuid].jsonl`
