...
# ///
"""Voice-fidelity judge for joe-coach.
...
def _build_persona() -> str:
    """Mirror the production config: SKILL.md operating core (frontmatter stripped)
    + the full voice library reference. This is what actually drives the coach when
    the skill triggers, so the eval measures production fidelity, not the raw prompt."""
    skill_md = (SKILL / "SKILL.md").read_text(encoding="utf-8")
    if skill_md.startswith("---"):
        skill_md = skill_md.split("---", 2)[2]  # drop YAML frontmatter (metadata)
    full = (SKILL / "references" / "coach-prompt-full.md").read_text(encoding="utf-8")
