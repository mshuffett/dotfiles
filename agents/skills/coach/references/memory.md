...
- **2026-06-28 (weekly review, 4/5):** Two improvements, both surfaced AND fixed in-session: (1) **lean into background execution** — push mechanical work (calendar edits, Todoist bulk ops, drafts, research) to background agents and keep the agenda moving; don't block. (2) **Make sure the Todoist inbox actually gets cleared** — it had been silently dropping every review. Both now structurally encoded: the *Non-Blocking Execution During Agendas* + two-stage review gate (SKILL.md) and the explicit "clear the Todoist inbox to zero" checklist line (weekly-review.md Phase 2 + don't-skip checklist). Treat both as standing behavior, not one-offs. Overall: good session — inbox→0, plan + focus system tight.

- **2026-06-29 (afternoon + evening shutdown):** (1) **AI-slop on copy.** When Michael asked for one-liner/positioning options, my generated list read as slop (mad-libs analogies "AWS for X / it's like LinkedIn", buzzword stacks). His own plain, spoken lines were far better — landed on *"we're building a network of increasingly autonomous companies."* **Rule:** on naming / taglines / positioning copy, do NOT spray generated option-lists — lead with HIS voice, mirror his plain+twist mechanic, and *sharpen his words* (sounding board), don't generate. He has a documented AI-slop allergy. (2) **Shutdown SOP fix:** renamed the confusing "execution score (0–2)" → **"Focus score: where did your main energy go — 2 = #1 / 1 = split / 0 = cathedral."** He never parsed the old phrasing. (3) **Trend (recurring):** first deep block started ~4pm again → the two investor-motion P2s (founder-dinner emails + investor email triage) slipped + an 11-hr day. Reinforce **first deep block in the AM**; tomorrow's TP1 is 10am — protect it. Good session overall: shipped v1 pitch skeleton + deck, consolidated the Notion pitch docs to one canonical home, locked the one-liner.
