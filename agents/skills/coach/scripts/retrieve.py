...
#   2. <skill-root>/corpus/index      — bundled inside the coach skill (self-contained)
#   3. ~/ws/notes/.../index           — the live vault corpus (refreshable)
# The bundled copy under the skill makes coach self-contained; the vault path stays
# as a refreshable fallback (the build pipeline writes there).
...
def _resolve_index() -> pathlib.Path:
    ...
    candidates.append(_SKILL_ROOT / "corpus" / "index")  # bundled in-skill
